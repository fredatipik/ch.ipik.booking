...
CREATE TABLE IF NOT EXISTS `civicrm_booking_therapist` (
  ...
  `max_advance_days` smallint unsigned NOT NULL DEFAULT 60 COMMENT 'Réservation max N jours à l\'avance',
...
CREATE TABLE IF NOT EXISTS `civicrm_booking_appointment_type` (
  ...
  `requires_account_creation` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Création compte WP obligatoire',
...
CREATE TABLE IF NOT EXISTS `civicrm_booking_appointment` (
  ...
  `gcal_event_id`        varchar(255) DEFAULT NULL COMMENT 'Google Calendar event ID (Phase 2)',
...
-- Valeurs par défaut
INSERT INTO `civicrm_booking_settings` (`key`, `value`) VALUES
